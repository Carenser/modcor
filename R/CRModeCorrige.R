        ...
        , .f = function(ref, deb, fin, meca, meth, entt, site, variante, signe, ts = tbl_cdc, prev = tbl_prev)
        {
          real = ts %>%
            dplyr::filter(
              ...
              , CODE_ENTITE == entt
              , CODE_SITE == site
              , HORODATE_UTC %within% as.list(deb%--%fin)
            )
          ...
          if(meth == 'HISTORIQUE')
          {
            ts %>%
              ...
              mutate(HORODATE_UTC = as_date(deb) + hour(HORODATE_UTC) + minute(HORODATE_UTC)) %>%
              ...
              transmute(MECANISME, CODE_ENTITE, CODE_SITE, HORODATE = HORODATE.x, HORODATE_UTC, REALISE = PUISSANCE.x, REFERENCE, PAS = PAS.x)
          }
          ...
          if(meth == 'RECTANGLE')
          {
            ...
            if(meca == 'NEBEF'){
              ...
              print(ref$PERIODE_REFERENCE[1])
              print(ref$PERIODE_REFERENCE[2])
