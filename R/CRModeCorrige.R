...
CRModeCorrige <- function(tbl_cdc, tbl_sites, tbl_eff, tbl_effhisto, tbl_entt, tbl_homol, tbl_indhist, tbl_prev){
  ...
  test = fuzzy_left_join(
    ...
    #dplyr::filter(METHODE =='RECTANGLE' & MECANISME == 'MA' & !is.na(CODE_SITE)) %>%
    split(list(.$MECANISME, .$CODE_ENTITE, .$CODE_SITE, .$METHODE, .$DEBUT, .$FIN), drop = TRUE) %>%
    {
      purrr::map_dfr(
        .x = .
        , tbl_ts = dplyr::filter(tbl_cdc, MECANISME == .$MECANISME, CODE_ENTITE == .$CODE_ENTITE, CODE_SITE == .$CODE_SITE)
        , .f = function(tbl_prog, tbl_ts)
        {
          ...
          mutate(tbl_prog, DEBUT_ETENDU = as_date(DEBUT,tz='CET') - if_else(METHODE == 'HISTORIQUE', days(60), days(1)), FIN_ETENDU = as_date(FIN,tz='CET') + days(2)) %>%
            #jointure avec les CdC sur la période nécessaire au calcul
            fuzzyjoin::fuzzy_left_join(
              y = mutate(tbl_ts, DATE = as_date(HORODATE, tz = 'CET'))
              , by = c('MECANISME','CODE_ENTITE','CODE_SITE', 'DEBUT_ETENDU' = 'DATE' , 'FIN_ETENDU' = 'DATE')
              , match_fun = list(`==`,`==`,`==`,`<=`,`>`)
            ) %>%
            transmute(
              MECANISME = MECANISME.x
              , CODE_ENTITE = CODE_ENTITE.x
              , CODE_SITE = CODE_SITE.x
              , TYPE_CONTRAT
              , CAPA_MAX_H_SITE
              , DEBUT
            ...
            group_by(MECANISME, CODE_ENTITE, CODE_SITE, TYPE_CONTRAT, CAPA_MAX_H_SITE, DEBUT, FIN, DMO, SIGNE, METHODE, VARIANTE) %>%
            nest() %>%
            {
              call(
                name = paste('CR', unique(tbl_prog[['METHODE']]), sep = '_') #application de la méthode de contrôle du réalisé de l'entité
                , eff = select(., - data)
                , cdc = select(., data) %>% unnest()
              ) %>%
                eval()
            }
        }
