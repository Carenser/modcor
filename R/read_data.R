          ...
          , .f = function(x,y,z){
            ...
            if(stringr::str_detect(string = x, pattern = "OA_GRD_([0-9]{8})_[0-9A-Z]{16}_([0-9]{14}).csv$")){ # Traitement des fichiers MA
              ...
              readr::read_delim(
                ...
                dplyr::group_by(CODE_ENTITE, DEBUT, FIN) %>% #On regroupe les OA par entité, début et fin
                ...
                dplyr::mutate(
                  DMO = if_else(
                    ...
                    , true = as.numeric(as.duration(DEBUT - lag(DEBUT) + lubridate::dminutes(lag(DMO))),'minutes')
                    , false = as.numeric(DMO)
            ...
            if(stringr::str_detect(string = x, pattern = "CRMA_[0-9]{4}_[0-9]{8}_[0-9]{6}_[0-9]{8}.csv")){ # Traitement des fichiers MA
              ...
              readr::read_delim(
                ...
                dplyr::mutate(
                  PAS = as.integer(
                    dplyr::case_when(
                      NB_PTS_CHRONIQUE %in% c(36,48,50) ~ 1800
                      , NB_PTS_CHRONIQUE %in% c(136,144,150) ~ 600
                      , NB_PTS_CHRONIQUE %in% c(276,288,300) ~ 300
                      , TRUE ~ NA_real_
                    )
                  )
                ) %>%
                dplyr::transmute(CODE_ENTITE, CODE_SITE, HORODATE,  HORODATE_UTC = lubridate::with_tz(HORODATE,tzone = 'UTC'), PUISSANCE, PAS) %>% #On crée une colonne horodate_UTC en conservant les autres colonnes nécessaires
              ...
              if(stringr::str_detect(string = x, pattern = "NEBEF_CRS_GRD_[0-9]{8}_[0-9A-Z]{16}_[0-9]{14}.csv")){ # Traitement des fichiers NEBEF
                ...
                readr::read_delim(
                  ...
                  dplyr::transmute(CODE_ENTITE, CODE_SITE, HORODATE,  HORODATE_UTC = with_tz(HORODATE,tzone = 'UTC'), PUISSANCE, PAS) %>% #On crée une colonne horodate_UTC en conservant les autres colonnes nécessaires
                  tibble::add_column(HORODATE_CREATION = z, MECANISME = y,.before = 1)
              }
            }
          }
...
#' Chargement des fichiers de courbes de prévision aux formats prévues dans les règles SI décrivant les flux en provenance des GRD à destination de RTE
#'
#' @param dossiers le(s) nom(s) de répertoire contenant les fichiers passés en paramètre (facultatif)
#' @param fichiers un vecteur contenant les noms des fichiers de courbes de prévision
#'
#' @return un dataframe comprenant 6 colonnes : MECANISME, CODE_ENTITE, CODE_SITE, HORODATE, HORODATE_UTC, PUISSANCE
#' @export
#' @import tidyverse
#' @import lubridate
#' @examples
read_prev<-function(dossiers, fichiers = NULL){
  ...
  if(!any(stringr::str_detect(string = fichiers,pattern = "^(NEBEF|MA)_PREV_GRD_[0-9A-Z]{16}_([0-9]{8})_([0-9]{14}).csv$")))
  {
    ...
    stringr::str_match(string =  fichiers, pattern = "^(NEBEF|MA)_PREV_GRD_[0-9A-Z]{16}_([0-9]{8})_([0-9]{14}).csv$") %>%
      ...
      dplyr::transmute(
        ...
        , mecanisme = V2
        , horodate_creation = ymd_hms(V4,tz = 'CET')
        , date_validite = as_date(V3)
      ) %>%
      dplyr::filter(str_detect(string = fichier, pattern = "^(NEBEF|MA)_PREV_GRD_[0-9A-Z]{16}_([0-9]{8})_([0-9]{14}).csv$")) %>% # On ne conserve que les fichiers de CdC
      dplyr::arrange(mecanisme, date_validite, desc(horodate_creation)) %>% # On trie les fichiers par mécanisme, période de validité et horodate de création
      dplyr::distinct(mecanisme, date_validite,.keep_all = TRUE) %>% # On ne conserve que le dernier fichier reçu pour une période donnée
      {
        purrr::pmap_dfr(
          ...
          , .f = function(x,y,z){
            ...
            if(y == 'MA'){
              ...
              readr::read_delim(
                ...
                dplyr::rename(CODE_ENTITE = CODE_EDA,CODE_SITE = CODE_EXT_SITE) %>% #On renomme la table avec des noms communs aux différents mécanismes
                tidyr::gather(- CODE_ENTITE, - CODE_SITE, - DATE, - NB_PTS_CHRONIQUE, key = 'MINUTE', value = 'PUISSANCE') %>% #On transpose la table en ligne
                dplyr::mutate(MINUTE = parse_integer(str_extract(string = MINUTE, pattern = '[0-9]+')) - 1) %>% # On interprète le nom de la colonne VAL en numérique en retranchant 1
                dplyr::filter(MINUTE < NB_PTS_CHRONIQUE) %>% #On filtre les points inutiles (VAL150, etc, ...)
                dplyr::mutate(MINUTE = (60 * 24 * MINUTE)/NB_PTS_CHRONIQUE) %>% #On convertit la valeur de VAL en minute
                ...
                dplyr::transmute(
                  ...
                  , CODE_SITE
                  , HORODATE
                  , HORODATE_UTC = lubridate::with_tz(HORODATE,tzone = 'UTC')
                  , PUISSANCE
                  , PAS = as.integer(dplyr::case_when(
                    NB_PTS_CHRONIQUE %in% c(36,48,50) ~ 1800
                    , NB_PTS_CHRONIQUE %in% c(136,144,150) ~ 600
                    , NB_PTS_CHRONIQUE %in% c(276,288,300) ~ 300
                    , TRUE ~ NA_real_
...
#' @import lubridate
#' @examples
read_entt<-function(dossiers, fichiers = NULL){
...
read_homol<-function(dossiers,fichiers = NULL){
  ...
  if(!any(stringr::str_detect(string = fichiers,pattern = "^(MA|NEBEF)_SITES_HOMOL_GRD_[0-9A-Z]{16}_([0-9]{14}).csv$")))
  {
    ...
    stringr::str_match(string =  fichiers, pattern = "^(MA|NEBEF)_SITES_HOMOL_GRD_[0-9A-Z]{16}_([0-9]{14}).csv$") %>%
      ...
      dplyr::distinct(mecanisme,.keep_all = TRUE) %>% # On ne conserve que le dernier fichier reçu pour une période donnée (pour NEBEF on ne conserve que le dernier fichier créé)
      {
        purrr::pmap_dfr(
          .l = list(
            x = str_c(.$dossier,.$fichier,sep='')
...
#' @import lubridate
#' @examples
read_indispo<-function(dossiers, fichiers = NULL){#on charge le dernier fichier Indispo connu
  ...
  if(!any(stringr::str_detect(string = fichiers,pattern = "^(MA|NEBEF)_INDHIST_GRD_[0-9A-Z]{16}_([0-9]{8})_([0-9]{14}).csv$")))
  {
    ...
    stringr::str_match(string =  fichiers, pattern = "^(MA|NEBEF)_INDHIST_GRD_[0-9A-Z]{16}_([0-9]{8})_([0-9]{14}).csv$") %>%
      ...
      dplyr::transmute(
        ...
        , fichier = V1
        , mecanisme = V2
        , date_validite = lubridate::as_date(V3)
        , horodate_creation = lubridate::ymd_hms(V4,tz = "CET")
      ) %>%
      dplyr::filter(str_detect(string = fichier, pattern =  "^(MA|NEBEF)_INDHIST_GRD_[0-9A-Z]{16}_([0-9]{8})_([0-9]{14}).csv$")) %>% # On ne conserve que les fichiers d'indisponibilité
      dplyr::arrange(mecanisme, date_validite, desc(horodate_creation)) %>% # On trie les fichiers par mécanisme, période de validité et horodate de création
      dplyr::distinct(mecanisme,date_validite,.keep_all = TRUE) %>% # On ne conserve que le dernier fichier reçu pour une période donnée (pour NEBEF on ne conserve que le dernier fichier créé)
      {
        purrr::pmap_dfr(
          ...
          , .f = function(x,y){
            suppressWarnings(
              readr::read_delim(
                ...
                , locale = locale(decimal_mark = ',',date_format = '%Y%m%d')
                , comment = '<EOF>'
                , col_names = c('CODE_ENTITE','CODE_SITE','DATE')
                , col_types =
                  list(
                    CODE_ENTITE = 'c'
                    , CODE_SITE = 'c'
              ...
              tibble::add_column(MECANISME = y,.before = 1) #On renomme la table avec des noms communs aux différents mécanismes en ajoutant la colonne mécanisme
          }
        )
      }
  }
