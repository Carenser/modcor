...
if(!is.null(fileLog))
{
  file.create(fileLog, showWarnings = FALSE)
  fileLogCon = file(description = fileLog, open = 'a+b')
  
  sink(file = fileLogCon, append = T, split = F, type = "message")
  sink(file = fileLogCon, append = T, split = T, type = "output")
}
...
stop("Aucun fichier à traiter. Supprimez les fichiers 'MODECOR' et relancez le script")
